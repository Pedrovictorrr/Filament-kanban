...
class UserResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        function getAvatarInput(): component
        {
            return  Section::make('Avatar')->description('Foto de perfil.')->schema([
                FileUpload::make('avatar_url')
                    ->image()
                    ->imageEditor()->label('Foto de perfil')
                    ->disk('local')
        ...
        return $form
            ->schema([
                getAvatarInput()->disabled(fn (Get $get) => $get('Permissão') == '1' && !Auth()->user()->hasRole('Super'))->visibleOn('edit'),
                getAvatarInput()->visibleOn('create'),
                getInformacaoPessoal()->visibleOn('create'),
                getInformacaoPessoal()->disabled(fn (Get $get) => $get('Permissão') == '1' && !Auth()->user()->hasRole('Super'))->visibleOn('edit'),
                Group::make([
                    getPasswordInput()->visibleOn('edit')->disabled(fn (Get $get) => $get('Permissão') == '1' && !Auth()->user()->hasRole('Super')),
                    getPasswordInput()->visibleOn('create'),
                ]),
                getPermissaoInput()->disabled(fn (Get $get) => $get('Permissão') == '1' && !Auth()->user()->hasRole('Super'))->visibleOn('edit'),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->query(User::query()->whereDoesntHave('roles', function ($query) {
                ...
                ImageColumn::make('foto')->disk('local')->square(),
